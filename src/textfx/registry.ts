import { EffectSpec } from './types';

export const EFFECTS_REGISTRY: Record<string, EffectSpec> = {
  neon: {
    id: 'neon',
    name: 'Neon / Glow',
    params: [
      { key: 'innerColor', type: 'color', default: '#FFFFFF' },
      { key: 'glowColor', type: 'color', default: '#00E5FF' },
      {
        key: 'glowRadius',
        type: 'number',
        min: 1,
        max: 64,
        step: 1,
        default: 18,
        unit: 'px',
      },
      {
        key: 'strokeWidth',
        type: 'number',
        min: 0,
        max: 16,
        step: 0.5,
        default: 2,
        unit: 'px',
      },
      { key: 'strokeColor', type: 'color', default: '#7DF9FF' },
    ],
  },
  softShadow: {
    id: 'softShadow',
    name: 'Soft Shadow',
    params: [
      { key: 'shadowColor', type: 'color', default: 'rgba(0,0,0,0.6)' },
      {
        key: 'offsetX',
        type: 'number',
        min: -50,
        max: 50,
        step: 1,
        default: 8,
        unit: 'px',
      },
      {
        key: 'offsetY',
        type: 'number',
        min: -50,
        max: 50,
        step: 1,
        default: 12,
        unit: 'px',
      },
      {
        key: 'blur',
        type: 'number',
        min: 0,
        max: 40,
        step: 1,
        default: 18,
        unit: 'px',
      },
    ],
  },
  longShadow: {
    id: 'longShadow',
    name: 'Long Shadow',
    params: [
      { key: 'shadowColor', type: 'color', default: 'rgba(0,0,0,0.7)' },
      {
        key: 'length',
        type: 'number',
        min: 4,
        max: 120,
        step: 4,
        default: 24,
        unit: 'px',
      },
      {
        key: 'angle',
        type: 'number',
        min: 0,
        max: 360,
        step: 5,
        default: 135,
        unit: '°',
      },
      {
        key: 'fade',
        type: 'number',
        min: 0,
        max: 1,
        step: 0.05,
        default: 0.6,
      },
    ],
  },
  bloomHalo: {
    id: 'bloomHalo',
    name: 'Bloom',
    params: [
      {
        key: 'threshold',
        type: 'number',
        min: 0,
        max: 1,
        step: 0.05,
        default: 0.6,
      },
      {
        key: 'radius',
        type: 'number',
        min: 0,
        max: 50,
        step: 1,
        default: 16,
        unit: 'px',
      },
      {
        key: 'intensity',
        type: 'number',
        min: 0,
        max: 1,
        step: 0.05,
        default: 0.75,
      },
    ],
  },
  glossy3d: {
    id: 'glossy3d',
    name: 'Glossy 3D',
    params: [
      { key: 'fillColorTop', type: 'color', default: '#FF6B9D' },
      { key: 'fillColorBottom', type: 'color', default: '#C51162' },
      { key: 'outerStrokeColor', type: 'color', default: '#000000' },
      {
        key: 'outerStrokeWidth',
        type: 'number',
        min: 2,
        max: 12,
        step: 1,
        default: 6,
        unit: 'px',
      },
      { key: 'innerStrokeColor', type: 'color', default: '#FFFFFF' },
      {
        key: 'innerStrokeWidth',
        type: 'number',
        min: 1,
        max: 8,
        step: 0.5,
        default: 3,
        unit: 'px',
      },
      {
        key: 'highlightIntensity',
        type: 'number',
        min: 0,
        max: 1,
        step: 0.05,
        default: 0.7,
      },
      {
        key: 'highlightHeight',
        type: 'number',
        min: 0.1,
        max: 0.5,
        step: 0.05,
        default: 0.25,
      },
      { key: 'shadowColor', type: 'color', default: 'rgba(0,0,0,0.4)' },
      {
        key: 'shadowOffsetY',
        type: 'number',
        min: 2,
        max: 20,
        step: 1,
        default: 6,
        unit: 'px',
      },
      {
        key: 'shadowBlur',
        type: 'number',
        min: 0,
        max: 16,
        step: 1,
        default: 4,
        unit: 'px',
      },
    ],
  },
  chrome3d: {
    id: 'chrome3d',
    name: 'Chrome 3D',
    params: [
      { key: 'metallicLight', type: 'color', default: '#E8E8E8' },
      { key: 'metallicMid', type: 'color', default: '#A0A0A0' },
      { key: 'metallicDark', type: 'color', default: '#505050' },
      {
        key: 'extrusionDepth',
        type: 'number',
        min: 4,
        max: 20,
        step: 1,
        default: 10,
        unit: 'px',
      },
      { key: 'extrusionColor', type: 'color', default: '#404040' },
      { key: 'outerStrokeColor', type: 'color', default: '#303030' },
      {
        key: 'outerStrokeWidth',
        type: 'number',
        min: 1,
        max: 8,
        step: 0.5,
        default: 3,
        unit: 'px',
      },
      { key: 'innerStrokeColor', type: 'color', default: '#D0D0D0' },
      {
        key: 'innerStrokeWidth',
        type: 'number',
        min: 0.5,
        max: 6,
        step: 0.5,
        default: 2,
        unit: 'px',
      },
      {
        key: 'shineIntensity',
        type: 'number',
        min: 0,
        max: 1,
        step: 0.05,
        default: 0.6,
      },
      {
        key: 'shineAngle',
        type: 'number',
        min: 0,
        max: 360,
        step: 15,
        default: 135,
        unit: '°',
      },
    ],
  },
};

export function getEffectDisplayName(id: string): string {
  return EFFECTS_REGISTRY[id]?.name || id; // never empty
}

export function getEffectSpec(id: string): EffectSpec | undefined {
  return EFFECTS_REGISTRY[id];
}
